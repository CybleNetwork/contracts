...
contract CrystalMagic is ERC20, AccessControlEnumerable {
    /* ========== DECLARE =============== */
    ...
    bytes32 public constant CMF_GOVERNANCE = keccak256("CMF_GOVERNANCE");
    bytes32 public constant CMF_MINTER = keccak256("CMF_MINTER");
    ...
    constructor() ERC20("Crystal Magic", "CM") {
        cap = 2100000000 ether; // Max Supply: 21 billion NIMF

        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(CMF_GOVERNANCE, msg.sender);
        _setupRole(CMF_MINTER, msg.sender);

        _mint(_msgSender(), cap);
        officialAccount = msg.sender;
    }

    function setCap(uint256 _newCap) external onlyRole(CMF_GOVERNANCE) {
        require(totalSupply() <= _newCap, "exceeds current supply");
        cap = _newCap;
    }

    function setMinter(address _account, bool _minter) external onlyRole(CMF_GOVERNANCE) {
        require(_account != address(0), "zero");
        minter[_account] = _minter;
        emit OnUpdateMinter(_account, _minter);
    }

    function setOfficialAccount(address _account) external onlyRole(CMF_GOVERNANCE) {
        require(_account != address(0), "zero");
    ...
    function mint(address _recipient, uint256 _amount) external onlyRole(CMF_MINTER) returns (bool) {
