...
Security
Insights
DemonDoge/contracts/DeamonDoge.sol
@Malnasser
Malnasser fix add
Latest commit 1d6c636 2 days ago
 History
 1 contributor
2323 lines (1979 sloc)  70.3 KB
...
/*
    Attempting fork of Tiki Token https://bscscan.com/address/0x9b76D1B12Ff738c113200EB043350022EBf12Ff0#code
    
   Setting 2% transaction fee = sent into the locked liquidity pool.
...
// part 1
contract DeamonDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Deamon Doge", "DDOGE") {
        ...
        marketingWallet = payable(0x483a332876694bFA0da7F683c09C1bF8f16B833c);
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "DDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "DDOGE: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        ...
        onlyOwner
    {
        require(
            ...
            "DDOGE: The liquidity wallet is already this address"
        );
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 sellTokens = _currentMarketingTokens
                .add(_currentBNBRewardsTokens)
                .div(100);
            swapAndSendMarketingTokensAndDividends(
                sellTokens,
                _currentMarketingTokens.div(100),
                _currentBNBRewardsTokens.div(100)
            );
        ...
        if (takeFee) {
            ...
            if (!sellFee) {
                ...
                _currentBNBRewardsTokens = _currentBNBRewardsTokens.add(
                    fees.mul(BNBRewardsFee)
                );
                _currentLiquidityTokens = _currentLiquidityTokens.add(
                    fees.mul(liquidityFee)
                );
                _currentMarketingTokens = _currentMarketingTokens.add(
                    fees.mul(marketingFee)
                );
            } else {
                fees = amount.mul(sellingTotalFees).div(100);
                //reset each current fee amount in wallet
                _currentBNBRewardsTokens = _currentBNBRewardsTokens.add(
                    fees.mul(sellingBNBRewardsFee)
                );
                _currentLiquidityTokens = _currentLiquidityTokens.add(
                    fees.mul(sellingLiquidityFee)
                );
                _currentMarketingTokens = _currentMarketingTokens.add(
                    fees.mul(sellingMarketingFee)
                );
            }
    ...
    function checkFees() private {
        ...
        uint256 totalTokens = _currentLiquidityTokens
            .add(_currentMarketingTokens)
            .add(_currentBNBRewardsTokens);

        if (balance != totalTokens) {
            if (
                _currentLiquidityTokens == 0 ||
                _currentMarketingTokens == 0 ||
                _currentBNBRewardsTokens == 0
            ) {
                _currentLiquidityTokens = balance.mul(liquidityFee).div(
                    totalFees
                );
                _currentMarketingTokens = balance.mul(marketingFee).div(
                    totalFees
                );
                _currentBNBRewardsTokens = balance
                    .sub(_currentLiquidityTokens)
                    .sub(_currentMarketingTokens);
            } else {
                _currentLiquidityTokens = balance
                    .mul(_currentLiquidityTokens)
                    .div(totalTokens);
                _currentMarketingTokens = balance
                    .mul(_currentMarketingTokens)
                    .div(totalTokens);
                _currentBNBRewardsTokens = balance
                    .sub(_currentLiquidityTokens)
                    .sub(_currentMarketingTokens);
    ...
    function swapAndSendMarketingTokensAndDividends(
        uint256 tokens,
        uint256 marketingTokens,
        uint256 BNBRewardsTokens
    ) private {
        uint256 initialBalance = address(this).balance;
        //swap token to bnb
        ...
        uint256 marketingBNB = address(this)
            .balance
            .sub(initialBalance)
            .mul(marketingTokens)
            .div(tokens);
