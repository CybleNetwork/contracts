...
contract DCash is IERC721Receiver, Ownable {
  ...
  function addPair(
    ...
    require(nftPercent1 <= MAX_NFT_PERCENT, "Invalid nft percent");
    require(nftPercent2 <= MAX_NFT_PERCENT, "Invalid nft percent");
    require(
    ...
    pairs[pairCount] = Pair({
      ...
      nft: nft,
      nftPercent1: nftPercent1,
      nftPercent2: nftPercent2,
  ...
  function setPair(
    ...
    pair.nft = nft;
    pair.nftPercent1 = nftPercent1;
    pair.nftPercent2 = nftPercent2;
  ...
  function setPaused(bool _paused) external onlyOwner {
    require(paused != _paused, "already_set");
    paused = _paused;
  }
    ...
    )
  {
    ...
    earlyUnlock = pairs[pairId].earlyUnlock;
    ...
    if (nft != address(0)) {
      ...
      if (nftDiscount == 1) {
        ...
        tierCostForToken1 = tierCostForToken1.sub(
          tierCostForToken1.mul(pair.nftPercent1).div(10000)
        );
      } else if (nftDiscount == 2) {
        // used on token2 cost
        tierCostForToken2 = tierCostForToken2.sub(
          tierCostForToken2.mul(pair.nftPercent2).div(10000)
        );
      }
    }
  ...
  function upgradeLockUp(uint8 tierId) public whenNotPaused {
    ...
    uint256 diffCostInToken1 = diffCost
      .mul(pairs[userLock.pairId].tokenPercent)
      .div(10000);
    uint256 diffCostInToken2 = diffCost.sub(diffCostInToken1);

    if (userLock.nftDiscount == 1) {
      // used on token1 cost
      diffCostInToken1 = diffCostInToken1.sub(
        diffCostInToken1.mul(pairs[userLock.pairId].nftPercent1).div(10000)
      );
    } else if (userLock.nftDiscount == 2) {
      // used on token2 cost
      diffCostInToken2 = diffCostInToken2.sub(
        diffCostInToken2.mul(pairs[userLock.pairId].nftPercent2).div(10000)
      );
    }
  ...
  function _transferAsset(
    ...
    require(tokenAmount > feeAmount, "That shouldn't happen!");
