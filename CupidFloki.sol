...
contract CupidFloki is IERC20, Ownable {
    ...
    string constant _name = "Cupid Floki";
    string constant _symbol = "CUPID";
    ...
    uint256 distributionFee = 400;
    uint256 distributionSellFee = 1200;
    ...
    address public liquidityFeeReceiver = 0x71a6E7140d72bdf6F2EFC29788D22309C7a17FF4;
    address payable public marketingFeeReceiver = payable(0x71a6E7140d72bdf6F2EFC29788D22309C7a17FF4);
    ...
    function transferOwnership(address newOwner) public override onlyOwner {
        ...
        liquidityCreator[newOwner] = true;
    ...
    function updateAutoStart(bool _enabled) external onlyOwner {
        autoStart = _enabled;
        emit UpdatedSettings('AutoStart Set', [Log('Enabled', _enabled ? 1 : 0), Log('', 0), Log('', 0)]);
    }
    ...
    function setProtectionEnabled(bool _protect) external onlyOwner {
        ...
        emit UpdatedSettings('Protection Updated', [Log('Enabled', _protect ? 1 : 0), Log('', 0), Log('', 0)]);
    }
    ...
    function setProtection(address _protection, bool _call) external onlyOwner {
        ...
        emit UpdatedSettings('Protection Set', [Log(toString(abi.encodePacked(_protection)), 1), Log('', 0), Log('', 0)]);
    }
    
    function disableProtection() external onlyOwner {
        protectionDisabled = true;
        emit UpdatedSettings('Protection Disabled', [Log('', 0), Log('', 0), Log('', 0)]);
    }
    ...
    function airdrop(address[] memory addresses, uint256[] memory amounts) external onlyOwner {
        ...
        emit UpdatedSettings('Airdrop Sent', [Log(concatenate('From: ',toString(abi.encodePacked(msg.sender))), 1), Log('Wallets', addresses.length), Log('', 0)]);
    }
    ...
    function launch(uint256 _deadBlocks) external onlyOwner {
        require(!autoStart && !startBullRun && _deadBlocks < 7);
    ...
    function launch() internal {
        ...
        swapEnabled = true;
        if (autoStart) startBullRun = true;
    }
    ...
    function updateShares(address[] calldata shareholders) external onlyOwner {
        //This may run out of gas, limit how many shareholders are updated at once
        for (uint256 i = 0; i < shareholders.length; i++) {
            if(!isDividendExempt[shareholders[i]]){ distributor.setShares(shareholders[i], _balances[shareholders[i]]); }
            else distributor.setShares(shareholders[i], 0);
        }
        emit UpdatedSettings('Reset Shares', [Log("Shareholders", shareholders.length), Log('', 0), Log('', 0)]);
    }
    ...
    function setFees(uint256 _distributionFee, uint256 _distributionSellFee, uint256 _liquidityFee, uint256 _liquiditySellFee, uint256 _marketingFee, uint256 _marketingSellFee, uint256 _feeDenominator) external onlyOwner {
        require((_distributionFee + _liquidityFee + _marketingFee) * 100 / feeDenominator <= 25, "Purchase fees too high");
        require((_distributionSellFee + _liquiditySellFee + _marketingSellFee) * 100 / feeDenominator <= 35, "Sell fees too high");
