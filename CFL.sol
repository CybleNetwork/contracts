...
contract CrytpoFantasyLeague is IERC20 {
    ...
    string constant private _name = "Crypto Fantasy League";
    string constant private _symbol = "CFL";
    ...
    address payable private _marketingWallet = payable(0x8d3c5F446331a42aA0e0CC7b3e8A6780dA7b4F7E);
...
//===============================================================================================================
    ...
    function setRatios(uint16 rewards, uint16 liquidity, uint16 marketing) external onlyOwner {
        require (rewards + liquidity + marketing >= 99, "Must add up to 100% or 99%.");
        _ratios.rewards = rewards;
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.total = liquidity + marketing + rewards;
    }
    ...
    function getMaxWalletSizeUI() public view returns (uint256) {
        return (((getCirculatingSupply() / (10**_decimals)) * maxWalletPercent) / maxWalletDivisor);
    }
    ...
    function setMaxTxPercent(uint256 percent, uint256 divisor) public onlyOwner {
        ...
        require(check >= (_tTotal * 2) / 1000, "Max Transaction amt must be above 0.2% of total supply.");
        _maxTxAmount = check;
    ...
    function liftFirstHourLimits() public onlyOwner {
        _taxRates.buyFee = 1000;
        _taxRates.sellFee = 1000;
        _taxRates.transferFee = 1000;
        setMaxTxPercent(1,1);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if(_hasLimits(from, to)) {
            ...
            if(_taxRates.sellFee > 2500) {
                if(block.timestamp > launchedAt + 1 hours) {
                    liftFirstHourLimits();
                }
            }
            ...
            if(to != _routerAddress && !lpPairs[to]) {
                require(balanceOf(to) + amount <= (maxWalletPercent * getCirculatingSupply()) / maxWalletDivisor, "Transfer amount exceeds the maxWalletSize.");
            }
        }
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        uint256 amountBNBReflection = ((amountBNB - amountBNBLiquidity) * _ratios.rewards) / (_ratios.marketing + _ratios.rewards);
        ...
        __balances.liquidity = amountBNBLiquidity;
        __balances.rewards = amountBNBReflection;
