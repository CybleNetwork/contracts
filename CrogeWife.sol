...
contract TestToken is ERC20, Auth {
    using SafeMath for uint256;
    ...
    event SellFeesChanged(uint256 _liquidityFee, uint256 _marketingFee, uint256 _operationsFee, uint256 _stakingFee, uint256 _rewardsFee);
    event BuyFeesChanged(uint256 _liquidityFee, uint256 _marketingFee, uint256 _operationsFee, uint256 _stakingFee, uint256 _rewardsFee);
    ...
    string constant private _name = "Croge Wife";
    string constant private _symbol = "CROW";
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(marketingFee).add(opsFee).add(stakingFee).add(rewardsFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeMarketing).add(sellFeeOps).add(stakingFee).add(sellFeeRewards);

    uint256 private feeDenominator  = 100;

    address private autoLiquidityReceiver;
    address private marketingFeeReceiver = 0xBd18C1F64212c4c89D830D5564b648c527Bd40F3;
    address private opsFeeReceiver = 0xa58abF4531E4633F8Cd893dbbca32a614F5C5980;
    address private stakingFeeReceiver = 0x5b35B7eC57DD6Ea307ddc7368388E851F61B62d6;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHReflection = amountETH.mul(rewardsFee.add(sellFeeRewards)).div(totalETHFee);
        uint256 amountETHOps = amountETH.sub(amountETHLiquidity).sub(amountETHMarketing).sub(amountETHReflection);

        try dividendDistributor.deposit{value: amountETHReflection}() {} catch {}

        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountETHMarketing}("");
        (tmpSuccess,) = payable(opsFeeReceiver).call{value: amountETHOps}("");
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _opsFee, uint256 _stakingFee, uint256 _rewardsFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        opsFee = _opsFee;
        stakingFee = _stakingFee;
        rewardsFee = _rewardsFee;
        totalBuyFee = _liquidityFee.add(_marketingFee).add(_opsFee).add(_stakingFee).add(_rewardsFee);
        feeDenominator = _feeDenominator;
        require(totalBuyFee <= maxFee, "Fees cannot be higher than 30%");

        emit BuyFeesChanged(_liquidityFee, _marketingFee, _opsFee, _stakingFee, _rewardsFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _opsFee, uint256 _stakingFee, uint256 _rewardsFee, uint256 _feeDenominator) external authorized {
        sellFeeLiquidity = _liquidityFee;
        sellFeeMarketing = _marketingFee;
        sellFeeOps = _opsFee;
        sellFeeStaking = _stakingFee;
        sellFeeRewards = _rewardsFee;
        totalSellFee = _liquidityFee.add(_marketingFee).add(_opsFee).add(_stakingFee).add(_rewardsFee);
        feeDenominator = _feeDenominator;
        require(totalSellFee <= maxFee, "Fees cannot be higher than 30%");

        emit SellFeesChanged(_liquidityFee, _marketingFee, _opsFee, _stakingFee, _rewardsFee);
    }
