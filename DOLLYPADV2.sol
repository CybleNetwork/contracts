...
contract DOLLYV2 is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "DOLLYPAD V2";
    string constant _symbol = "DOLLYV2";
    ...
    uint256 public buyEcosystemFee = 2;
    ...
    address public marketingFeeReceiver = 0x4Ae595a017e2Af261991165B93169BB4cCF07700;
    address public ecosystemFeeReceiver = 0x9754e4dEd5275050C3D57d3355494b8eD70AB2A2;
    ...
    function updateBuyFees(
        ...
        buyBurnFee = burn;
        buyTotalFees = reward.add(marketing).add(liquidity).add(ecosystem).add(burn);
    ...
    function updateSellFees(
        ...
        sellBurnFee = burn;
        sellTotalFees = reward.add(marketing).add(liquidity).add(ecosystem).add(burn);
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 totalSwapFee = buyMarketingFee.add(buyEcosystemFee);
        uint256 marketingFeeBnb = swappedBnbAmount.mul(buyMarketingFee).div(
            totalSwapFee
        );
        uint256 ecosystemFeeBnb = swappedBnbAmount.sub(marketingFeeBnb);
        ...
        if (ecosystemFeeBnb > 0) {
            (bool ecosystemSuccess, ) = payable(ecosystemFeeReceiver).call{
                value: ecosystemFeeBnb,
                gas: 30000
            }("");
            // only to supress warning msg
            ecosystemSuccess = false;
        }
    ...
    function swapAndSendFees(uint256 tokensForFee) private {
        ...
        uint256 ecosystemToken = currentTokenBalance.sub(marketingToken);
        ...
        if (ecosystemToken > 0) {
            _approve(address(this), ecosystemFeeReceiver, ecosystemToken);
            IERC20(SWAPTOKEN).transfer(ecosystemFeeReceiver, ecosystemToken);
            emit SendFeesInToken(ecosystemFeeReceiver, ecosystemToken);
        }
    }
    ...
    function setFeeReceivers(address _marketingFeeReceiver, address _ecosystemFeeReceiver) external onlyOwner {
